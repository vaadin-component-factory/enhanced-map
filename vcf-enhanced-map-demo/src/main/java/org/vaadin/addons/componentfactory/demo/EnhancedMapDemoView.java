...
public class EnhancedMapDemoView extends VerticalLayout {
    ...
    public EnhancedMapDemoView() {
        ...
        initMap();      
        Div mapPlaceHolder = new Div(map);
        mapPlaceHolder.setSizeFull();
        add(mapPlaceHolder);
        ...
        Checkbox cb = new Checkbox("Draw only one polygon", ev-> {
        	onePolygon = !onePolygon;
        	mapPlaceHolder.remove(map);
        	map = new EnhancedMap(new DrawingLayerOptions("rgba(0,255,0,0.4)","purple", 3, onePolygon));
        	initMap();
        	mapPlaceHolder.add(map);
        });
        
        add(new HorizontalLayout(draw, remove, removeAll, cb)); 
